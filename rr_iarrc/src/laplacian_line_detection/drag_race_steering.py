...
def getMidlineY(img, points1, points2, poly_deg=1, n_points=100, plot=True, color=(0, 255, 0)):
    ...
    midy = [(combined1[i][0] + combined2[i][0]) / 2 for i in range(n_points)]
...
def callback(data):
    ...
    cv2.line(img, (size/2, 0), (size/2, size), 0, 2)
    ...
    for cnt in contours:
        ...
        if bottommost[0] > size / 2 and bottommost[1] > lowestRightPnt:
            lowestRightPnt = bottommost[1]
            rightCnt = cnt
        elif bottommost[0] < size / 2 and bottommost[1] > lowestLeftPnt:
    ...
    if len(leftLinePoints) != 0 and len(rightLinePoints) != 0:
        ...
        steering_angle = 0
    o2 = time.time()
    steering_angle = min(math.radians(steering_angle), .4)
